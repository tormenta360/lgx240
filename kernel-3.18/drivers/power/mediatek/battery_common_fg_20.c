...
/* ////////////////////////////////////////////////////////////////////////////// */
//<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]Increase debug level
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	// + 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Modify charging and discharging current #1
	bool current_state;
	// - 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Modify charging and discharging current #1
	...
	case POWER_SUPPLY_PROP_batt_temp:
		// <<< 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Switch On/OFF JEITA #3
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		// + 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Modify charging and discharging current #1
		current_state=battery_meter_get_battery_current_sign();
		if( current_state == 0) {
			//discharging
			val->intval = (-1)*(data->BAT_BatteryAverageCurrent/10);
		} else {
			// + 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Modify charging and discharging current
			//val->intval = data->BAT_BatteryAverageCurrent;
			val->intval = data->BAT_BatteryAverageCurrent/10;
			// - 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Modify charging and discharging current
		}
		// - 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Modify charging and discharging current #1
...
void mt_battery_GetBatteryData(void)
{
	...
	//<2016/09/16-nickygao, [LV3][MISC][COMMON][BSP][][]Get charging and discharging current from HW
	#if 1
	...
	//<2016/09/16-nickygao, [LV3][MISC][COMMON][BSP][][]Test mode
	if (battery_cmd_thermal_test_mode == 1) 
	{
	        battery_log(BAT_LOG_CRTI, "[Battery] In thermal_test_mode, battery temperature is fixed.\n");
	}
	else
	{
...
#else
	//<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]Update the battery warning message
...
*/
		...
		battery_log(BAT_LOG_CRTI, "[BAT_thread]Cable out \r\n");
              //<2016/09/15-nickygao, [LV3][MISC][COMMON][BSP][][]Charging IC will not charging without reset
              {
...
// + 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Show Instantaneous current
static ssize_t show_BatteryCurrent(struct device *dev, struct device_attribute *attr,
                    const char *buf, size_t size)
{
	int ICharging, current_state;
	ICharging = battery_meter_get_charging_current();
	current_state=battery_meter_get_battery_current_sign();
	#if 0
	if( current_state == 0) {
		//discharging
		ICharging = (-1)*(ICharging/10);
	} else {
...
static DEVICE_ATTR(BatteryCurrent, 0664, show_BatteryCurrent, NULL);
// - 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Show Instantaneous current
...
static int mt_batteryNotify_probe(struct platform_device *dev)
{
	...
	// + 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Show Instantaneous current
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_BatteryCurrent);
	// - 20160926-Rody, [LV3][MISC][COMMON][BSP][][]Show Instantaneous current
