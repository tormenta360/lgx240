...
// <<< 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Switch On/OFF JEITA
//unsigned int g_temp_switch = 1;
// <<< 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Switch On/OFF JEITA
...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
static ssize_t show_temp_switch(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	battery_log(BAT_LOG_CRTI, "temp_switch = %d\n", BMT_status.temperswitch_enable);
	return sprintf(buf, "%u\n", BMT_status.temperswitch_enable);
}

static ssize_t store_temp_switch(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	if (kstrtouint(buf, 10, &BMT_status.temperswitch_enable) == 0) {
...
static void mt_battery_notify_VBatTemp_check(void)
{
	// <<< 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Switch On/OFF JEITA #3
	if(BMT_status.temperswitch_enable == 1) {
		printk("%s :Into temperswitch_enable=%d temperature= %d", __func__,BMT_status.temperswitch_enable, BMT_status.temperature);
	// >>> 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Switch On/OFF JEITA #3
		#if defined(BATTERY_NOTIFY_CASE_0002_VBATTEMP)
		// <<< 20160920_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Add Battery Notify above 60 centigrage
		...
		// <<< 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Add Battery Notify at 57 centigrade and Exit Power Notify at 55 centigrade
		//	if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) {
		if (BMT_status.temperature >= BATTERY_NOTIFY_TEMPERATURE) {
			g_BatteryNotifyCode |= 0x0002;
			battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too high)\n",
				    BMT_status.temperature);
		}
		if (BMT_status.temperature >= BATTERY_EXITPOWER_NOTIFY_TEMPERATURE && BMT_status.charger_exist == KAL_TRUE) {
			g_BatteryNotifyCode |= 0x0002;
			battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too high)\n",
				    BMT_status.temperature);
		}
	...
	// <<< 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Switch On/OFF JEITA #3
		...
		printk("%s :Out temperswitch_enable=%d temperature= %d", __func__,BMT_status.temperswitch_enable, BMT_status.temperature);
	}
	// >>> 20160908_Rody-[LV3][FEATURE][COMMON][BATTERY][][]Switch On/OFF JEITA #3
}
...
static void mt_battery_thermal_check(void)
{
	...
	/* ignore default rule */
#else
	// <<< 20160907-[LV3][PORTING][COMMON][BATTERY][][]Add power off temperature
		if ((BMT_status.temperature >= 60) && BMaaT_status.temperswitch_enable==1) {
			printk("%s : Into temperswitch_enable=%d temperature= %d", __func__,BMT_status.temperswitch_enable, BMT_status.temperature);
	// >>> 20160907-[LV3][PORTING][COMMON][BATTERY][][]Add power off temperature
	#if defined(CONFIG_POWER_EXT)
				battery_log(BAT_LOG_CRTI,
	...
	#else
			{
				if ((g_platform_boot_mode == META_BOOT)
					...
					// <<< 20160920 Rody -[LV3][PORTING][COMMON][BATTERY][][]Close power off over temperature
					#if 0
					bat_data->BAT_CAPACITY = 0;
