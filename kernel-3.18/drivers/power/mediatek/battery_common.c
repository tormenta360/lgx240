...
/* ////////////////////////////////////////////////////////////////////////////// */
//<2016/10/25-nickygao, [LV3][MISC][COMMON][BSP][][]Increase debug level
...
#ifdef ARIMA_THERMAL_CONTROL_CURRENT
int ARIMA_THERMAL_CONTROL = BAT_LOG_FULL;
#endif
// - 20161107 Rody [D6][FEATURE][COMMON][BATTERY][][] Thermal Charger control

//<add by JieZhang 20170210
extern int arima_lcm_status_int_flag;
...
//<2016/10/25-nickygao, [LV3][FEATURE][COMMON][BSP][][]Battery OTP test
 unsigned int g_batt_lgotp_status = TEMP_POS_NORMAL;
//<2016/10/25-nickygao
...
//<2016/12/04-kevin,add cycle charging
int Cycle_charging_flag = -1;
//>2016/12/04-kevin

//<2016/12/24-kevin,add cycle charging range set as 45%~50%
int cycle_mode_flag = -1;
//>2016/12/24-kevin
...
static enum power_supply_property battery_props[] = {
	...
	// + 20161114-Rody, [LV3][PORTING][COMMON][BATTERY][][] Add instant Battery temperature node
	POWER_SUPPLY_PROP_instant_batt_temp,
	// - 20161114-Rody, [LV3][PORTING][COMMON][BATTERY][][] Add instant Battery temperature node
	/* Add for EM */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	// + 20161114-Rody, [LV3][PORTING][COMMON][BATTERY][][] Add instant Battery temperature node
	unsigned int instant_batt_temp;
	// - 20161114-Rody, [LV3][PORTING][COMMON][BATTERY][][] Add instant Battery temperature node
	...
	case POWER_SUPPLY_PROP_batt_temp:
		// + 20160921-Rody, [LV3][PORTING][COMMON][BATTERY][][]create temp_switch node
		if (BMT_status.temperswitch_enable == 0) {
		...
		break;
	// + 20161114-Rody, [LV3][PORTING][COMMON][BATTERY][][] Add instant Battery temperature node
	case POWER_SUPPLY_PROP_instant_batt_temp:
		instant_batt_temp = battery_meter_get_battery_temperature();
		val->intval = instant_batt_temp;
		battery_log(BAT_LOG_FULL, "instant battery temperature is %d, %d\n", val->intval, instant_batt_temp);
		break;
	// - 20161114-Rody, [LV3][PORTING][COMMON][BATTERY][][] Add instant Battery temperature node
...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
/* // Create File to contol charge mode by thermal */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
// + 20161107 Rody [D6][FEATURE][COMMON][BATTERY][][] Thermal Charger control
#ifdef ARIMA_THERMAL_CONTROL_CURRENT
static ssize_t show_arima_thermal_control(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	unsigned int write_data;
	write_data = BMT_status.charge_option_mode;
	battery_log(ARIMA_THERMAL_CONTROL, "[Thermal Control][%s]: write data= %d \n",
					__func__, write_data);
	return sprintf(buf, "%u\n", write_data);
}

static ssize_t store_arima_thermal_control(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	unsigned int read_data;
	
	kstrtouint(buf, 10, &read_data);
	battery_log(ARIMA_THERMAL_CONTROL, "[Thermal Control][%s]: read data= %d \n",
					__func__, read_data);
	BMT_status.charge_option_mode = read_data;
	return size;
}

static DEVICE_ATTR( arima_thermal_control, 0664, show_arima_thermal_control, store_arima_thermal_control);
#endif
// - 20161107 Rody [D6][FEATURE][COMMON][BATTERY][][] Thermal Charger control
/* ///////////////////////////////////////////////////////////////////////////////////////// */
...
//<2017/2/21-nickygao, [LV3][MISC][COMMON][BSP][][]MTK original bug, will cause TD9815 and TD8889, so design same as D5
#if 0
		if (BMT_status.UI_SOC == 100)
...
//<2017/2/21-nickygao
		...
		if (BMT_status.UI_SOC >= 99) {
			...
			//<2017/2/20-nickygao, [LV3][MISC][COMMON][BSP][][]Fix the side effect of TD9185 and TD8889
			//resetBatteryMeter = KAL_FALSE;
			resetBatteryMeter = KAL_TRUE;
			//<2017/2/20-nickygao
...
//<2016/12/04-kevin,add cycle charging
static ssize_t show_Cycle_charging(struct device *dev, struct device_attribute *attr,char *buf)
{
	unsigned int write_Cycle_charging_data;
	write_Cycle_charging_data = Cycle_charging_flag;
	printk("[Cycle_charging][%s]: write Cycle_charging data= %d\n",__func__, write_Cycle_charging_data);
	return sprintf(buf, "%u\n", write_Cycle_charging_data);
}

static ssize_t store_Cycle_charging(struct device *dev, struct device_attribute *attr,const char *buf, size_t size)
{
	unsigned int read_Cycle_charging_data;
	
	kstrtouint(buf, 10, &read_Cycle_charging_data);
	printk("[Cycle_charging][%s]: read Cycle_charging data= %d\n",__func__, read_Cycle_charging_data);
	Cycle_charging_flag = read_Cycle_charging_data;
	return size;
}
static DEVICE_ATTR(Cycle_charging, 0664, show_Cycle_charging, store_Cycle_charging);
//>2016/12/04-kevin.

//<2016/12/24-kevin,add cycle charging range set as 45%~50%	
static ssize_t show_cycle_mode(struct device *dev, struct device_attribute *attr,char *buf)
{
	unsigned int write_cycle_mode_data;
	write_cycle_mode_data = cycle_mode_flag;
	printk("[cycle_mode][%s]: write cycle_mode data= %d\n",__func__, write_cycle_mode_data);
	return sprintf(buf, "%u\n", write_cycle_mode_data);
}

static ssize_t store_cycle_mode(struct device *dev, struct device_attribute *attr,const char *buf, size_t size)
{
	unsigned int read_cycle_mode_data;
	
	kstrtouint(buf, 10, &read_cycle_mode_data);
	printk("[cycle_mode][%s]: read cycle_mode data= %d\n",__func__, read_cycle_mode_data);
	cycle_mode_flag = read_cycle_mode_data;
	return size;
}
static DEVICE_ATTR(cycle_mode, 0664, show_cycle_mode, store_cycle_mode);
//>2016/12/24-kevin.
...
#if 1
    if (BMT_status.temperature >= MAX_SECOND_CHARGE_TEMPERATURE && BMT_status.temperature < MAX_CHARGE_TEMPERATURE) {
    	    battery_log(BAT_LOG_CRTI,
		    "[BATTERY] do_batt_temp_state_machine Battery Temperature is more than 45l !!\n\r");
	    if (BMT_status.bat_vol >= MAX_SECOND_T_BAT_V) 
	    {
	        battery_log(BAT_LOG_CRTI,
	                    "[BATTERY] do_batt_temp_state_machine Battery Vbat >= 4Vl !!\n\r");
            
	        g_batt_lgotp_status = TEMP_POS_HIGH;
		 return PMU_STATUS_FAIL;
	    } 
           else
	    {
	         ...
	         g_batt_lgotp_status = TEMP_POS_NORMAL;
		  BMT_status.bat_charging_state = CHR_PRE;
	         return PMU_STATUS_OK;
	    }
	
    } else if (g_batt_lgotp_status == TEMP_POS_HIGH) {
               if(BMT_status.temperature <= MAX_SECOND_CHARGE_TEMPERATURE_MINUS_X_DEGREE) {
                
			g_batt_lgotp_status = TEMP_POS_NORMAL;
			BMT_status.bat_charging_state = CHR_PRE;
	    	        battery_log(BAT_LOG_CRTI,
		                "[BATTERY] do_batt_temp_state_machine Battery is recoverd from 45C!\n\r");
			return PMU_STATUS_OK;
	       } else {
			return PMU_STATUS_FAIL;
               }
...
//<2016/12/04-kevin,add cycle charging and add cycle charging range set as 45%~50%
	printk("[test] Cycle_charging_flag = %d,cycle_mode_flag = %d\n",Cycle_charging_flag,cycle_mode_flag);
	if(Cycle_charging_flag == 1 && cycle_mode_flag == 0) {
		printk("[test] new feature charging mode Current BMT_status.UI_SOC = %d\n",BMT_status.UI_SOC);
		if (BMT_status.UI_SOC >= 95) {
			battery_log(BAT_LOG_FULL,"BMT_status.UI_SOC >= 95 and current mode is = 0x%0x,new feature stop charging\n",BMT_status.bat_charging_state);
			return PMU_STATUS_FAIL;
		} else if (BMT_status.UI_SOC <= 5) {			
			battery_log(BAT_LOG_FULL,"BMT_status.UI_SOC <= 5 and current mode is = 0x%0x,new feature start charging\n",BMT_status.bat_charging_state);
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
 		} else {
			if(BMT_status.bat_charging_state == CHR_PRE) {
				battery_log(BAT_LOG_FULL,"because pre_status = 0x%0x is discharging,stop!\n",BMT_status.bat_charging_state);
				return PMU_STATUS_FAIL;
			} else {
				battery_log(BAT_LOG_FULL,"because pre_status = 0x%0x is not discharging,continues!\n",BMT_status.bat_charging_state);
				return PMU_STATUS_OK;
			}
		}	
	}	
	else if(cycle_mode_flag == 1 && Cycle_charging_flag == 0) {
		printk("[test2] cycle mode Current BMT_status.UI_SOC = %d\n",BMT_status.UI_SOC);
		if (BMT_status.UI_SOC >= 50) {
			battery_log(BAT_LOG_FULL,"BMT_status.UI_SOC >= 50 and current mode is = 0x%0x,cycle mode stop charging\n",BMT_status.bat_charging_state);
			return PMU_STATUS_FAIL;
		} else if (BMT_status.UI_SOC <= 45) {			
			battery_log(BAT_LOG_FULL,"BMT_status.UI_SOC <= 45 and current mode is = 0x%0x,cycle mode start charging\n",BMT_status.bat_charging_state);
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
		} else {
			if(BMT_status.bat_charging_state == CHR_PRE) {
	...
	else if (cycle_mode_flag == 1 && Cycle_charging_flag == 1) {
			printk("[test3] new feature charging mode Current BMT_status.UI_SOC = %d\n",BMT_status.UI_SOC);
		if (BMT_status.UI_SOC >= 95) {
			battery_log(BAT_LOG_FULL,"BMT_status.UI_SOC >= 95 and current mode is = 0x%0x,new feature stop charging\n",BMT_status.bat_charging_state);
			return PMU_STATUS_FAIL;
		} else if (BMT_status.UI_SOC <= 5) {			
...
//<2016/12/15-nickygao, [LV3][MISC][COMMON][BATTERY][][]Reduce quantity of battery temperature to optimize updating time
static void custom_mt_battery_average_method_init(BATTERY_AVG_ENUM type, unsigned int *bufferdata,
					   unsigned int data, signed int *sum)
{
	unsigned int i;
	static kal_bool batteryBufferFirst = KAL_TRUE;
	...
	if (batteryBufferFirst == KAL_TRUE) {
		for (i = 0; i < CUSTOM_BATTERY_AVERAGE_SIZE; i++)
			bufferdata[i] = data;


		*sum = data * CUSTOM_BATTERY_AVERAGE_SIZE;
	}

	index++;
	if (index >= CUSTOM_BATTERY_AVERAGE_DATA_NUMBER) {
		index = CUSTOM_BATTERY_AVERAGE_DATA_NUMBER;
		batteryBufferFirst = KAL_FALSE;
	}
}
//<2016/12/15-nickygao
...
//<2016/12/15-nickygao, [LV3][MISC][COMMON][BATTERY][][]Reduce quantity of battery temperature to optimize updating time
static unsigned int custom_mt_battery_average_method(BATTERY_AVG_ENUM type, unsigned int *bufferdata,
					    unsigned int data, signed int *sum,
					    unsigned char batteryIndex)
{
	unsigned int avgdata;

	custom_mt_battery_average_method_init(type, bufferdata, data, sum);

	*sum -= bufferdata[batteryIndex];
	*sum += data;
	bufferdata[batteryIndex] = data;
	avgdata = (*sum) / CUSTOM_BATTERY_AVERAGE_SIZE;
...
void mt_battery_GetBatteryData(void)
{
	...
	#else
	static signed int custom_batteryTempBuffer[CUSTOM_BATTERY_AVERAGE_SIZE];
	static unsigned char custom_batteryIndex;
	#endif
	//<2016/12/15-nickygao
	...
	//<2016/12/1-nickygao, [LV3][MISC][COMMON][BSP][][]Add charging current indicator from HW FG
	#if 1    /* legacy */
	if (upmu_is_chr_det() == KAL_TRUE)
	...
	//<2016/12/1-nickygao, [LV3][MISC][COMMON][BSP][][]we do not have Rsense but use HW FG
	#if 0
	...
	if (battery_cmd_thermal_test_mode == 1) {
		...
		#else
		BMT_status.temperature =
		    custom_mt_battery_average_method(BATTERY_AVG_TEMP, &custom_batteryTempBuffer[0], temperature,
					      &temperature_sum, custom_batteryIndex);
		#endif
		//<2016/12/15-nickygao
	}
	...
	if (batteryIndex >= BATTERY_AVERAGE_SIZE)
       ...
       //<2016/12/15-nickygao, [LV3][MISC][COMMON][BATTERY][][]Reduce quantity of battery temperature to optimize updating time
	custom_batteryIndex++;
	if (custom_batteryIndex >= CUSTOM_BATTERY_AVERAGE_SIZE)
		custom_batteryIndex = 0;          
       //<2016/12/15-nickygao
...
#else
	// - 20160921-Rody, [LV3][PORTING][COMMON][BATTERY][][]create temp_switch node
	printk("[%s] temperature= %d, temperswitch_enable= %d\n", 
			 __func__ ,BMT_status.temperature, BMT_status.temperswitch_enable);
	// - 20160921-Rody, [LV3][PORTING][COMMON][BATTERY][][]create temp_switch node

	//<2016/10/25-nickygao, [LV3][FEATURE][COMMON][BSP][][]Update the battery warning message
	//The message code is the same with LO2
	// >60
	if (BMT_status.temperature >= MAX_BAT_TEMPERATURE ) {
	// + 20160922 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
//		g_BatteryNotifyCode |= 0x0020;
		g_BatteryNotifyCode |= 0x0040;
	// - 20160922 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of shut down range\n",
				    BMT_status.temperature);
	}
	//<2016/10/25-nickygao
...
{
	// >57
	 if(BMT_status.temperature >= ALERT_BAT_TEMERATURE ) {
		// + 20161006 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
		// + 20160922 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
		g_BatteryNotifyCode |= 0x0002;
		// - 20160922 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
		// - 20161006 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of alert range\n",
				    BMT_status.temperature);
        }
}
else if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE ) {
		// + 20160922 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
		// + 20160921 Rody, [LV3][PORTING][COMMON][BATTERY][][]change node name
//		g_BatteryNotifyCode |= 0x0040;
		g_BatteryNotifyCode |= 0x0020;
		// - 20160921 Rody, [LV3][PORTING][COMMON][BATTERY][][]change node name
		// - 20160922 Rody, [LV3][PORTING][COMMON][BATTERY][][] Modify notify temperature
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of charging range\n",
				    BMT_status.temperature);
}	
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
		...
		battery_charging_control(CHARGING_CMD_GET_CHARGER_TYPE, &CHR_Type_num);
		// + Test Rody
		battery_log(BAT_LOG_FULL, "[%s]: charger_type = %d \n",	__func__, CHR_Type_num);
		// - Test Rody
		BMT_status.charger_type = CHR_Type_num;
...
*/
		...
		battery_log(BAT_LOG_FULL, "[BAT_thread]Cable out \r\n");
              //<2016/10/25-nickygao, [LV3][MISC][COMMON][BSP][][]Charging IC will not charging without reset
              {
...
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if (upmu_is_chr_det() == KAL_TRUE) {//charge in interrupt
#else
...
#endif
//<add by JieZhang 20170210
			if (mt_get_bl_brightness() == 0){ //0: BL is off before charge in 
				arima_lcm_status_int_flag = 0;
			}else{							  // 1:BL have already on before charge in 
				arima_lcm_status_int_flag = 1;
			}
			
			battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task] arima_lcm_status_int_flag = %d\n", arima_lcm_status_int_flag);
//>add by JieZhang 20170210
...
//>add by JieZhang 20170210
		...
		if (BMT_status.bat_vol > 0) {
			mt_battery_update_status();
			//<2016/12/15-nickygao, [LV3][MISC][COMMON][BSP][][]Fix the bug that plugging in/out TA quickly can cause charging status error
...
//->20170112 stevenzhang .[D6][]add 5mins usb mode
extern int arima_cycle_flag;
extern int arima_cycle_count;
extern int arima_1mins_switch_charge_count;    // 1 mins 
extern int arima_1mins_switch_charge_count1;
extern int arima_1mins_switch_charge_flag;    // 1 mins 
...
//<2017/01/23-samhuang, improve DQA heat emission issue
enum mt_cpu_dvfs_id {
	MT_CPU_DVFS_LITTLE,
	NR_MT_CPU_DVFS,
};
extern void mt_cpufreq_set_max_freq(enum mt_cpu_dvfs_id id, unsigned int freq);
extern unsigned int gIsCPUFeqMAX1000;
//>2017/01/23-samhuang
...
void BAT_thread(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		//<2017/01/23-samhuang, improve DQA heat emission issue
		mt_cpufreq_set_max_freq(0, 1248000);
		gIsCPUFeqMAX1000 = 0;
		//>2017/01/23-samhuang
    	//->20170112 stevenzhang .[D6][]add 5mins usb mode
    	arima_cycle_count = 0;
    	arima_cycle_flag = 1;
		arima_usb100_usb500_cycle_flag = 1;
		battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging],BAT_thread,arima_cycle_count= %d,arima_cycle_flag=%d \n",arima_cycle_count,arima_cycle_flag);
		arima_1mins_switch_charge_count = 0;	// 1 mins 
		arima_1mins_switch_charge_count1 = 0;
		arima_1mins_switch_charge_flag = 1;    // 1 mins 
		battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging],BAT_thread,arima_1mins_switch_charge_count= %d,arima_1mins_switch_charge_flag=%d \n",arima_1mins_switch_charge_count,arima_1mins_switch_charge_flag);
        //stevenzhang
		}
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		//<2016/12/04-kevin,add cycle charging
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Cycle_charging);
		//>2016/12/04-kevin.
		//<2016/12/24-kevin,add cycle charging range set as 45%~50% 
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_cycle_mode);
		//>2016/12/24-kevin.
		...
		// + 20161107 Rody [D6][FEATURE][COMMON][BATTERY][][] Thermal Charger control
		#ifdef ARIMA_THERMAL_CONTROL_CURRENT
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_arima_thermal_control);
		#endif
		// - 20161107 Rody [D6][FEATURE][COMMON][BATTERY][][] Thermal Charger control
	...
	// + 20161107 Rody [D6][FEATURE][COMMON][BATTERY][][] Thermal Charger control
	#ifdef ARIMA_THERMAL_CONTROL_CURRENT
	BMT_status.charge_option_mode=0;
	BMT_status.charge_option_times=0;
	BMT_status.charge_option_usb500mode_times=0;
	BMT_status.charge_option_tamode_times=0;
	#endif
	// - 20161107 Rody [D6][FEATURE][COMMON][BATTERY][][] Thermal Charger control
...
//<2016/10/25-nickygao, [LV3][MISC][COMMON][BSP][][]Initialize charger IC rt9536
...
static int battery_dts_probe(struct platform_device *dev)
{
	...
	if (ret) {
        ...
        //<2016/10/25-nickygao, [LV3][MISC][COMMON][BSP][][]Initialize charger IC rt9536
        charger_ic_rt96536_initialize(dev);
        //<2016/10/25-nickygao
