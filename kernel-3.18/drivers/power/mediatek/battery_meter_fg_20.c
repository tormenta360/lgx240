...
//<2016/09/15-nickygao, [LV3][MISC][COMMON][BSP][][]Increase debug level
...
// + 20161012- Rody, [D6][BUG][COMMON][ALARM][][] Add Fuel Gauge Information by FAQ17692
...
#endif
//<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]add temparature diable in hidden manu.
...
//<2016/09/16-nickygao, [LV3][MISC][COMMON][BSP][][]Get charging and discharging current from HW
	...
	return 0;
        ...
        ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CURRENT, &ICharging);
...
static ssize_t show_temp_switch(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	if (temperature_flag ==KAL_FALSE) {
		//FALSE =0
		printk("Rody: temp_switch = KAL_FALLSE, BMT_status.temperswitch_enable= %d\n", BMT_status.temperswitch_enable);
		write_data =1;
	} else if( temperature_flag ==KAL_TRUE) {
		// TRUE =1
		printk("Rody: temp_switch = KAL_TRUE, BMT_status.temperswitch_enable= %d\n", BMT_status.temperswitch_enable);
		write_data =0;
	}
...
static ssize_t show_temp_flag_on(struct device *dev,struct device_attribute *attr, char *buf)
{    
    temperature_flag=KAL_FALSE;
    
    
    bm_print(BM_LOG_CRTI, "[show_temp_flag_on] temperature_flag = %d\n", temperature_flag);
    
    
    return sprintf(buf, "%d\n", 0);
}
static DEVICE_ATTR(temp_on, 0444, show_temp_flag_on, NULL);
static ssize_t show_temp_flag_off(struct device *dev,struct device_attribute *attr, char *buf)
{    
    temperature_flag=KAL_TRUE;
    
    
    bm_print(BM_LOG_CRTI, "[show_temp_flag_off] temperature_flag = %d\n", temperature_flag);
   
   
    return sprintf(buf, "%d\n", 1);
}
static DEVICE_ATTR(temp_off, 0444, show_temp_flag_off, NULL);
//<2016/03/19-nickygao
#endif
// - 20160921-Rody, [LV3][PORTING][COMMON][BATTERY][][]change node name
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	#else
	//<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]Create dirver file temp_on and temp_off for hidden menu interface.
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_temp_on);
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_temp_off);
	//<2016/09/15-nickygao
	#endif
	// - 20160921-Rody, [LV3][PORTING][COMMON][BATTERY][][]change node name
