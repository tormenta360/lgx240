...
//-20170112 stevenzhang
int arima_cycle_flag = 1;
int arima_cycle_count = 0;
int arima_1mins_switch_charge_count = 0;    // 1 mins 
int arima_1mins_switch_charge_count1 = 0;    // 1 mins 
int arima_1mins_switch_charge_flag = 1;    // 1 mins 
#define  ARIMA_THERMAl_DEFAULT   0             //default davidda
//end
//<add by xuqin 2016/01/23
int arima_usb100_usb500_cycle_flag = 1;
int arima_lcm_status_int_flag = 0;              // 1:BL have already on before charge in     //0: BL is off before charge in 
//>add by xuqin
 /* ///////////////////////////////////////////////////////////////////////////////////////// */
 /* // PUMP EXPRESS */
 /* ///////////////////////////////////////////////////////////////////////////////////////// */
...
//<2016/12/23-nickygao, [LV3][FEATURE][COMMON][BSP][][]Thermal control
static void set_thermal_control(void)
{
        if (BMT_status.charge_option_mode == 4) 
	 {
	         g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
		  g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;
		  battery_log(BAT_LOG_CRTI, "[%s]com=%d charging cur=%d\n", __func__, BMT_status.charge_option_mode, g_temp_CC_value);
	 }
	 else if (BMT_status.charge_option_mode == 0)
	 {
	         #if 0
	 ...
	 {
	        battery_log(BAT_LOG_CRTI, "[%s]com=%d\n", __func__, BMT_status.charge_option_mode);
	 }
}
//<2016/12/23-nickygao

//<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]Battery OTP test
static void set_LG_OTP_charging_current(void)
{
    if (BMT_status.temperature > MAX_SECOND_CHARGE_TEMPERATURE && BMT_status.temperature < MAX_CHARGE_TEMPERATURE)
    {
	     ...
	     battery_log(BAT_LOG_CRTI, "[BATTERY] LG OTP set charging current : %d\r\n", g_temp_CC_value);
    }
}
//<2016/09/15-nickygao
...
unsigned int gIsCPUFeqMAX1000 = 0;
unsigned int gCurrentModeAfter10min = 0; // 1->  100ma 500ma switch  2->  500ma 850ma switch
extern u32 kbasep_get_gl_utilization(void);
extern unsigned int gGPUAVG;
//>2017/01/23-samhuang

//<2017/01/23-samhuang, Modify from 50 to 55 via LGE request
//<add by Jiezhang 20170219 
#define GPU_LOADING_THRESHOLD  55//50
//>add by Jiezhang 20170219
//>2017/01/23-samhuang
...
//<2017/1/1--davidda, enable thermal control
#if 0
		if (g_bcct_value < 100)
			g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA;
...
//->20170112 stevenzhang .[D6][]add 5mins usb mode
			      // 5 mins Usb --->( 1mins iset ----->1mins usb )
				/**When inserting the charger, the first 5 minutes for the USB mode, followed by B mode and USB mode cycle switch, switching time is 1 minute***/
				    if ( arima_cycle_flag == 1 ) {
						arima_cycle_count++;
						if(mt_get_bl_brightness() == 0){
							arima_cycle_count = 61;
						}
				    }
					
					battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging] 5 mins select_charging_current_bcct,arima_cycle_count=%d \n",arima_cycle_count);
					//<2017/01/23-samhuang, improve DQA heat emission issue   BL off before charge in
					if( arima_cycle_count == 61 /*32*/ ) { //10 mins 
					//>2017/01/23-samhuang
							arima_cycle_count = 0;
				    		//arima_cycle_flag = ~arima_cycle_flag;
				    		arima_cycle_flag = 0;
				    		//<2017/02/03-samhuang, add for 2 mins 100 mA <-->  1 min 390 mA
				    		mt_cpufreq_set_max_freq(0, 1248000);
				    		gIsCPUFeqMAX1000 = 0;
				    		arima_1mins_switch_charge_count = 0;
				    		//>2017/02/03-samhuang
				    		arima_1mins_switch_charge_count1 = 0;
				    		battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging] 5 mins arima_cycle_flag=%d \n",arima_cycle_flag);
					}
					
					//<2017/01/23-samhuang, improve DQA heat emission issue
					#if 1
					//if ( ( gIsCPUFeqMAX1000 == 0 ) && ( arima_cycle_flag == 1 )  && ( kbasep_get_gl_utilization() >= 90) ) {
					if ( ( gIsCPUFeqMAX1000 != 1 ) && ( arima_cycle_flag == 1 )  && ( gGPUAVG >= GPU_LOADING_THRESHOLD) ) {
						    mt_cpufreq_set_max_freq(0, 1027000);
							gIsCPUFeqMAX1000 = 1;
							gCurrentModeAfter10min = 1;
					}
					else if ( ( gIsCPUFeqMAX1000 == 0 ) && ( arima_cycle_flag == 1 )  && ( gGPUAVG < GPU_LOADING_THRESHOLD) ){
						    gIsCPUFeqMAX1000 = 2;
							gCurrentModeAfter10min = 2;
					}
					battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging]gCurrentModeAfter10min = %d \n",gCurrentModeAfter10min);	    
					
 					battery_log(BAT_LOG_CRTI, "GPU loading <gGPUAVG> is = %d \n",gGPUAVG);
 					#else
 					//battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging]  freq=%d \n", curr_gpufreq);
					if ( (arima_cycle_count == 1) && ( arima_cycle_flag == 1 ) )  {
							mt_cpufreq_set_max_freq(0, 1027000);
							gIsNeedUSB100 = 1;
					}
					if( (arima_cycle_count == 61) && (arima_cycle_flag != 1) ) {
						mt_cpufreq_set_max_freq(0, 1248000);
						gIsNeedUSB100 = 0;
					}
					#endif
					...
					if (g_bcct_value < 100){
						...
						if((arima_cycle_flag == 1)&&(arima_lcm_status_int_flag != 0)){     // 5 mins  usb
							//<2017/02/03-samhuang, add for 2 mins 100 mA <-->  1 min 390 mA
							#if 1
							if( gIsCPUFeqMAX1000 == 1) {//gpu > GPU_LOADING_LEVER
								if ( arima_1mins_switch_charge_count == 12 ) {  // 2 mins
									g_temp_CC_value = CHARGE_CURRENT_500_00_MA;  // 1 min 390 mA
								} else if ( arima_1mins_switch_charge_count == 18 ) { //3 mins
									g_temp_CC_value = CHARGE_CURRENT_100_00_MA;
									arima_1mins_switch_charge_count = 0;
								} else if ( arima_1mins_switch_charge_count == 0) {
									g_temp_CC_value = CHARGE_CURRENT_100_00_MA;
								}
								arima_1mins_switch_charge_count++;
							} else if( gIsCPUFeqMAX1000 == 2){//gpu < GPU_LOADING_LEVER
							      if ( arima_1mins_switch_charge_count1 == 14 ) {  // 1.5 mins
									g_temp_CC_value = CHARGE_CURRENT_850_00_MA;  
								} else if ( arima_1mins_switch_charge_count1 == 20 ) { //3 1mins
									g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
									arima_1mins_switch_charge_count1 = 0;
								} else if ( arima_1mins_switch_charge_count1 == 0) {
									g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
								}
								arima_1mins_switch_charge_count1++;
							}
							else {
								g_temp_CC_value = CHARGE_CURRENT_850_00_MA;
							}
							...
							if( (arima_cycle_count-1)%3 == 0 ) {
								if(g_temp_CC_value==CHARGE_CURRENT_500_00_MA)
							     	g_temp_CC_value = CHARGE_CURRENT_100_00_MA;
								else if(g_temp_CC_value==CHARGE_CURRENT_100_00_MA)
								 	g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
								else 
									g_temp_CC_value=CHARGE_CURRENT_100_00_MA;
							}
							#endif
							//>2017/02/03-samhuang
						}else{												// cycle 1mins Iset --> 1mins usb
							if(mt_get_bl_brightness() == 0) {//full charging when backlight off
								g_temp_CC_value = CHARGE_CURRENT_850_00_MA; 						
							}else{						//switch 850&500 when backlight on           Gpu>=GPU_LOADING_LEVER
								//if((( gGPUAVG >= GPU_LOADING_THRESHOLD) && (gCurrentModeAfter10min==2)) || (gCurrentModeAfter10min==1)){
									if(arima_1mins_switch_charge_count == 9){ //500ma: 90s   850ma:60s
										g_temp_CC_value = CHARGE_CURRENT_850_00_MA;		           			
									}else if(arima_1mins_switch_charge_count == 15){
										g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
										arima_1mins_switch_charge_count = 0;
									}else if(arima_1mins_switch_charge_count == 0){
										g_temp_CC_value = CHARGE_CURRENT_500_00_MA;	
									...
									arima_1mins_switch_charge_count ++; 						
									battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging] 1mins select_charging_current_bcct,arima_1mins_switch_charge_count=%d \n",arima_1mins_switch_charge_count);
								//}else{
								//	g_temp_CC_value = CHARGE_CURRENT_850_00_MA; 
								//}
							}
						}
					}else{        // g_bcct_value>= 850
						g_temp_CC_value = CHARGE_CURRENT_850_00_MA;
					}
			#endif     // ARIMA_THERMAl_DEFAULT
//->20170112 stevenzhang
#endif
//>2017/1/1--davidda
	} else {
	...
	battery_log(BAT_LOG_CRTI, "[thermal_cycle_charging] select_charging_current_bcct,g_bcct_value=%d g_temp_CC_value= %d \n",g_bcct_value,g_temp_CC_value);
...
*/
#endif
    //<2017/02/10-nickygao, [LV3][MISC][COMMON][BSP][][]Battery OTP test
    set_LG_OTP_charging_current();
    //<2016/02/10-nickygao
...
//->20170112 stevenzhang .[D6][]add 5mins usb mode
EXPORT_SYMBOL(arima_cycle_flag);
EXPORT_SYMBOL(arima_cycle_count);
EXPORT_SYMBOL(arima_1mins_switch_charge_count);
EXPORT_SYMBOL(arima_1mins_switch_charge_count1);
EXPORT_SYMBOL(arima_1mins_switch_charge_flag);

//->20170112 stevenzhang
//<add by xuqin 2017/01/23
EXPORT_SYMBOL(arima_usb100_usb500_cycle_flag);
//>add by xuqin
...
#ifndef CONFIG_MTK_SWITCH_INPUT_OUTPUT_CURRENT_SUPPORT
//<2017/1/1--davidda, enable thermal control
//<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]Only for test, don't use bcct
#if 1
...
//20170122  stevenzhang [LV3][],add USB100 mode
EXPORT_SYMBOL(g_bcct_flag);
//20170122  stevenzhang
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
	...
	pchr_turn_on_charging();
       ...
       //<2016/12/1-nickygao, [LV3][MISC][COMMON][BSP][][]Add a full battery condition that battery voltage must reach full battery voltage
       #if 0    /* MTK default */
	if (charging_full_check() == KAL_TRUE) {
	#else
	if (charging_full_check() == KAL_TRUE && BMT_status.bat_vol >= batt_cust_data.full_battery_voltage) {
	    battery_log(BAT_LOG_CRTI, "[%s]bat_vol(%d) >= full_battery_voltage(%d)\n", __func__, BMT_status.bat_vol, batt_cust_data.full_battery_voltage);
	#endif
	//<2016/12/1-nickygao
		BMT_status.bat_charging_state = CHR_BATFULL;
		BMT_status.bat_full = KAL_TRUE;
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	//<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]Stop charging while battery is full
	unsigned int charging_enable = KAL_FALSE;
	//<2016/09/15-nickygao
	...
	if (  //<2016/09/15-nickygao, [LV3][FEATURE][COMMON][BSP][][]Enable recharging
              /*charging_full_check() == KAL_FALSE */
		BMT_status.bat_vol < batt_cust_data.recharging_voltage || charging_full_check() == KAL_FALSE 
		//<2016/09/15-nickygao
