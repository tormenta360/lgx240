...
int g_temp_status = TEMP_NEG_10_TO_POS_45;
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	// <<< 20160920 Rody, Modify JEITA for LV3 SPEC
		// > 60
	if (g_temp_status == TEMP_ABOVE_POS_60) {
		cv_voltage = JEITA_TEMP_ABOVE_POS_60_CV_VOLTAGE;
	} // 55 ~ 60
	  else if (g_temp_status == TEMP_POS_55_TO_POS_60) {
		cv_voltage = JEITA_TEMP_POS_55_TO_POS_60_CV_VOLTAGE;
	} // 45 ~ 55
	 else if (g_temp_status == TEMP_POS_45_TO_POS_55) {
		cv_voltage = JEITA_TEMP_POS_45_TO_POS_55_CV_VOLTAGE;
	} // -10 ~ 45
	 else if (g_temp_status == TEMP_NEG_10_TO_POS_45) {
		cv_voltage = JEITA_TEMP_NEG_10_TO_POS_45_CV_VOLTAGE;
	} // < -10
	...
	// >>> 20160920 Rody, Modify JEITA for LV3 SPEC
	printk("switch_charging(%s): g_temp_status= %d, cv_voltage= %d",__func__, g_temp_status, cv_voltage);
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	// <<< 20160912 Rody [D6][PORTING][COMMON][BATTERY][][] add property to enable disable JEITA function
	if(BMT_status.temperswitch_enable == 1) {
	// >>> 20160912 Rody [D6][PORTING][COMMON][BATTERY][][] add property to enable disable JEITA function
		/* JEITA battery temp Standard */
		// <<< 20160920 Rody, Modify JEITA for LV3 SPEC
		   // > 60
		if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
			battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Over high Temperature(%d) !!\n\r", TEMP_POS_60_THRESHOLD);
			g_temp_status = TEMP_ABOVE_POS_60;
			return PMU_STATUS_FAIL;
		} // 55 ~ 60
		 else if (BMT_status.temperature > TEMP_POS_55_THRESHOLD) {	/* control 55c to normal behavior */
			if ((g_temp_status == TEMP_ABOVE_POS_60)
			    && (BMT_status.temperature >= TEMP_POS_60_THRES_MINUS_X_DEGREE)) {
				battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Temperaturea recovery between %d and %d, not allow charging yet!!\n\r",
							TEMP_POS_60_THRESHOLD, TEMP_POS_60_THRES_MINUS_X_DEGREE);
				jeita_status = PMU_STATUS_FAIL;
				// No Action from "> 60" recovery "55~60"
			} else {
				battery_log(BAT_LOG_CRTI, "[BATTERY] Battery Temperature between %d and %d !!\n\r", TEMP_POS_55_THRESHOLD, TEMP_POS_60_THRESHOLD);
				g_temp_status = TEMP_POS_55_TO_POS_60;
			}
		} // 45~55
		 else if (BMT_status.temperature >= TEMP_POS_45_THRESHOLD) {
			if ((g_temp_status == TEMP_POS_55_TO_POS_60 || g_temp_status == TEMP_POS_60_THRESHOLD)
			     && (BMT_status.temperature >= TEMP_POS_55_THRES_MINUS_X_DEGREE)) {
				battery_log(BAT_LOG_CRTI, "[BATTERY]1 Battery Temperature recovery  between %d and %d, current only 500mA!!\n\r",
						    TEMP_POS_55_THRESHOLD, TEMP_POS_55_THRES_MINUS_X_DEGREE);
				// No Action from "55~60" recovery "45~55"
			} else {
				battery_log(BAT_LOG_CRTI, "[BATTERY]2 Battery Normal Temperature between %d and %d !!\n\r",
				    	TEMP_POS_45_THRESHOLD, TEMP_POS_55_THRESHOLD);
				g_temp_status = TEMP_POS_45_TO_POS_55;
			}
		} // -10 ~ 45
		 else if (BMT_status.temperature >= TEMP_NEG_10_THRESHOLD) {
			if ((g_temp_status == TEMP_POS_45_TO_POS_55 || g_temp_status == TEMP_POS_55_TO_POS_60 || g_temp_status== TEMP_POS_60_THRESHOLD)
				&& (BMT_status.temperature >= TEMP_POS_45_THRES_MINUS_X_DEGREE)) {
				if (g_temp_status == TEMP_POS_60_THRESHOLD) {
					battery_log(BAT_LOG_CRTI, "[BATTERY]3 Battery Temperature recovery  between %d and %d, current 0mA!!\n\r",
									TEMP_POS_60_THRESHOLD, TEMP_POS_45_THRES_MINUS_X_DEGREE);
				}
				 else if (g_temp_status == TEMP_POS_55_TO_POS_60) {
					battery_log(BAT_LOG_CRTI, "[BATTERY]4 Battery Temperature recovery  between %d and %d, current 0mA!!\n\r",
									TEMP_POS_55_THRESHOLD, TEMP_POS_45_THRES_MINUS_X_DEGREE);
				}
				 else if (g_temp_status == TEMP_POS_45_TO_POS_55) {
					battery_log(BAT_LOG_CRTI, "[BATTERY]5 Battery Temperature recovery  between %d and %d, current 500mA!!\n\r",
									TEMP_POS_45_THRESHOLD, TEMP_POS_45_THRES_MINUS_X_DEGREE);
				}
				// No Action from "55~60" recovery "45 ~-10"
			}
			 else if ((g_temp_status == TEMP_BELOW_NEG_10 
			 		  && (BMT_status.temperature <= TEMP_NEG_10_THRES_PLUS_X_DEGREE))) {
						battery_log(BAT_LOG_CRTI, "[BATTERY]6 Battery Temperature recovery  between %d and %d, current only 500mA!!\n\r",
						    TEMP_NEG_10_THRESHOLD, TEMP_NEG_10_THRES_PLUS_X_DEGREE);
				// No Action from "<-10" recovery "45 ~-10"
				jeita_status = PMU_STATUS_FAIL;
				return PMU_STATUS_FAIL;
			}
			 else {
				battery_log(BAT_LOG_CRTI, "[BATTERY]7 Battery Temperature between %d and %d !!\n\r",
					    TEMP_NEG_10_THRESHOLD, TEMP_POS_45_THRESHOLD);
				g_temp_status = TEMP_NEG_10_TO_POS_45;
			}
		} // < -10
		  else {
			battery_log(BAT_LOG_CRTI, "[BATTERY]8 Battery below low Temperature(%d) !!\n\r",
	...
	// <<< 20160912 Rody [D6][PORTING][COMMON][BATTERY][][] add property to enable disable JEITA function
				...
				battery_log(BAT_LOG_CRTI,
					    "[BATTERY] temperature switch enalbe = %d, g_temp_status= %d!!\n\r",
				    	BMT_status.temperswitch_enable, TEMP_NEG_10_TO_POS_45);
				g_temp_status = TEMP_NEG_10_TO_POS_45;
...
// <<< 20160908 Rody,[LV3][FUTURE][COMMON][BATTERY][][]JEITA for LV3 SPEC
#if 1
	// <<< 20160920 Rody, Modify JEITA for LV3 SPEC
		// > 60
	if (g_temp_status == TEMP_ABOVE_POS_60) {
	...
	printk("switch_charging(%s): g_temp_CC_value= %d, g_temp_input_CC_value= %d",__func__, g_temp_CC_value, g_temp_input_CC_value );
	// >>> 20160920 Rody, Modify JEITA for LV3 SPEC
#else
...
void set_usb_current_unlimited(bool enable)
{
	...
	if (enable == true)
		...
		en = 0;
        // <<< 2016/08/03-youchihwang, [LV3][PORTING][COMMON][BATTERY][][]Porting charger, battery, rt9536 has no safety timer
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA || g_temp_input_CC_value == CHARGE_CURRENT_0_00_MA) {
			...
			// <<< 2016/08/03-youchihwang, [LV3][PORTING][COMMON][BATTERY][][]Porting charger, battery, rt9536 doesn't support set input current function.
			...
			battery_log(BAT_LOG_CRTI, "[BATTERY] g_temp_CC_value=%d !\r\n",g_temp_CC_value);

			/*Set CV Voltage */
#if !defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
			if (batt_cust_data.high_battery_voltage_support)
                // <<< 2016/08/03-youchihwang, [LV3][PORTING][COMMON][BATTERY][][]Porting charger, battery, Setting charging voltage if no support jeita.
...
{
        // <<< 2016/08/03-youchihwang, [LV3][PORTING][COMMON][BATTERY][][]Porting charger, battery, rt9536 has no watch dog timer
	...
	case CHR_ERROR:
        ...
        // <<< 2016/08/03-youchihwang, [LV3][PORTING][COMMON][BATTERY][][]Porting charger, battery, rt9536 has no register to read.
