...
//<2016/12/1-nickygao, [LV3][MISC][COMMON][BSP][][]Update full battery voltage according to battery temperature
...
//<2016/10/25-nickygao, [LV3][MISC][COMMON][BSP][][]Decrease debug level for optimizing power-on speed
...
//<2016/12/24-nickygao, [LV3][BUG][COMMON][BATTERY][TD8586][]Fix this bug
static int SW_OCV_with_charger = 0;
static int SW_SOC_with_charger = 0;
static int SW_SOC_save_with_charger = 0;
//<2016/12/24-nickygao
...
//<2016/12/17-nickygao, [LV3][MISC][COMMON][BSP][][]Add indicator for initial fuel gauge
static int HW_OCV = 0;
static int HW_SOC = 0;
static int g_vbat_capacity = 0;
static int SW_SOC = 0;
static kal_bool charger_exist = 0;
static char bootbuf_dod[190];
//<2016/12/17-nickygao
//<2016/11/08-nickygao, [LV3][MISC][COMMON][BSP][][]Set swocv as power-on voltage
static int g_tbat = 0;
//<2016/11/08-nickygao
...
#endif
//<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
#if defined(TEMPERATURE_T_N20)	
       batt_meter_cust_data.temperature_t_n20 = TEMPERATURE_T_N20;
#endif
//<2016/10/24-nickygao
...
#endif
//<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
#if defined(Q_MAX_NEG_20)	
       batt_meter_cust_data.q_max_neg_20 = Q_MAX_NEG_20;
#endif
//<2016/10/24-nickygao
...
#endif
//<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
#if defined(Q_MAX_NEG_20_H_CURRENT)	
       batt_meter_cust_data.q_max_neg_20_h_current = Q_MAX_NEG_20_H_CURRENT;
#endif
//<2016/10/24-nickygao
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	while (!of_property_read_u32_index(np, "batt_temperature_table", idx, &addr)) {
        ...
        //<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
        #if 0
        __batt_meter_parse_node(np, "battery_profile_t_n20_num", &num);
        
        __batt_meter_parse_table(np, "battery_profile_t_n20",
			fgauge_get_profile(batt_meter_cust_data.temperature_t_n20));
	#endif
        //<2016/10/24-nickygao
	...
	__batt_meter_parse_table(np, "battery_profile_t3",
        ...
        //<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
        #if 0
        __batt_meter_parse_node(np, "r_profile_t_n20_num", &num);

	__batt_meter_parse_table(np, "r_profile_t_n20",
			(BATTERY_PROFILE_STRUCT *)fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t_n20));
	#endif
        //<2016/10/24-nickygao
	...
	__batt_meter_parse_node(np, "q_max_neg_10",
        ...
        //<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set Q_MAX with -20 degree
        #if 0
        __batt_meter_parse_node(np, "q_max_neg_20",
		&batt_meter_cust_data.q_max_neg_20);
	#endif
        //<2016/10/24-nickygao
	...
	__batt_meter_parse_node(np, "q_max_pos_0_h_current",
        ...
        __batt_meter_parse_node(np, "q_max_neg_20_h_current",
		&batt_meter_cust_data.q_max_neg_20_h_current);
	#endif
        //<2016/10/24-nickygao
...
int BattThermistorConverTemp(int Res)
{
	...
	//<2016/10/26-nickygao, [LV3][FEATURE][COMMON][BSP][][]Battery OTP test to get battery temperature
	if (Res >= Batt_Temperature_Table[0].TemperatureR) {
		TBatt_Value = -20;
	} else if (Res <= Batt_Temperature_Table[85].TemperatureR) {
		TBatt_Value = 65;
	} else {
		RES1 = Batt_Temperature_Table[0].TemperatureR;
		TMP1 = Batt_Temperature_Table[0].BatteryTemp;
...
//<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
        ...
        if (temperature <= batt_meter_cust_data.temperature_t0) {                                                                                                                                                            
                low_temperature = (-20);
		low_Q_max = batt_meter_cust_data.q_max_neg_20;
		high_temperature = batt_meter_cust_data.temperature_t0;
		high_Q_max = batt_meter_cust_data.q_max_neg_10;

		if (temperature < low_temperature) {
			temperature = low_temperature;
		}
        } else if (temperature <= batt_meter_cust_data.temperature_t1) {
                low_temperature = batt_meter_cust_data.temperature_t0;
...
//<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
        ...
        if (temperature <= batt_meter_cust_data.temperature_t0) {                                                                                                                                                            
                low_temperature = (-20);
		low_Q_max = batt_meter_cust_data.q_max_neg_20_h_current;
		high_temperature = batt_meter_cust_data.temperature_t0;
		high_Q_max = batt_meter_cust_data.q_max_neg_10_h_current;
...
#endif
//<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Add temparature disable in hidden menu
kal_bool temperature_flag = KAL_FALSE;
//<2016/10/24-nickygao
...
int BattVoltToTemp(int dwVolt)
{
    ...
    //<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Add temparature disable in hidden menu
    if (temperature_flag) {
        bm_print(BM_LOG_CRTI, "[BattVoltToTemp]temperature_flag = %d \n", temperature_flag);
        return 25;
    } else {
	return sBaTTMP;
    }
...
{
//<2016/10/24-nickygao, [LV3][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
        if (temperature == batt_meter_cust_data.temperature_t_n20)
		return &battery_profile_t_n20[0];
//<2016/10/24-nickygao
	if (temperature == batt_meter_cust_data.temperature_t0)
...
#if 1
        if (temperature <= batt_meter_cust_data.temperature_t0) {
                low_profile_p = fgauge_get_profile(batt_meter_cust_data.temperature_t_n20);
		high_profile_p = fgauge_get_profile(batt_meter_cust_data.temperature_t0);
		low_temperature = (-20);
		high_temperature = batt_meter_cust_data.temperature_t0;
...
//<2016/10/24-nickygao, [N8][FEATURE][COMMON][BSP][][]Set charging with -20 degree ZCV table
#if 1
        if (temperature <= batt_meter_cust_data.temperature_t0) {
                low_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t_n20);
		high_profile_p = fgauge_get_profile_r_table(batt_meter_cust_data.temperature_t0);
		low_temperature = (-20);
		high_temperature = batt_meter_cust_data.temperature_t0;
...
//<2017/2/22-nickygao, [LV3][BUG][COMMON][BATTERY][TD7831][]Fix the bug of TD7831
#define REBOOT_TOLERANCE		7
#define POWER_ON_OFF_TOLERANCE		3
//<2017/2/22-nickygao
void dod_init(void)
{
#if defined(SOC_BY_HW_FG)
...
if (((g_rtc_fg_soc != 0)
...
#endif
	{
       ...
       //<2016/12/17-nickygao, [LV3][BUG][COMMON][BATTERY][TD7831][]Battery level error 
       int ret = 0;
       int plugout_status = 0;
       ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_BATTERY_PLUG_STATUS, &plugout_status);
       //<2016/12/17-nickygao
	if (((g_rtc_fg_soc != 0)
		...
		//<2017/2/22-nickygao, [LV3][BUG][COMMON][BATTERY][TD7831][]Fix the bug of TD7831
		#if 1
              if ((get_boot_reason() == BR_WDT_BY_PASS_PWK || get_boot_reason() == BR_WDT
		       || get_boot_reason() == BR_TOOL_BY_PASS_PWK || get_boot_reason() == BR_2SEC_REBOOT
		       || get_boot_mode() == RECOVERY_BOOT))
              {
                      if ((g_rtc_fg_soc> REBOOT_TOLERANCE + SW_SOC) || (g_rtc_fg_soc <= REBOOT_TOLERANCE))
                      {
                              g_rtc_fg_soc--;
                      }	
		}
	       ...
	       {
                      if (g_rtc_fg_soc > POWER_ON_OFF_TOLERANCE + HW_SOC)
	               {
	                       g_rtc_fg_soc--;
	               }	  	       
...
//<2017/2/18-nickygao, [LV3][MISC][COMMON][BSP][][]Add indicator for initial fuel gauge
bm_print(BM_LOG_FULL, "[dod_init][V0222]chr_exist=%d|boot_reason=%d|SW_OCV=%d SW_SOC=%d|SW_OCV_w=%d SW_SOC_w=%d|HW_OCV=%d HW_SOC=%d|RTC=%d|t=%d|%d %d|bat=%d\n",
		 charger_exist, get_boot_reason(), g_booting_vbat, SW_SOC, SW_OCV_with_charger, SW_SOC_save_with_charger, HW_OCV, HW_SOC, g_rtc_fg_soc, g_tbat, 
		 batt_meter_cust_data.cust_poweron_delta_capacity_tolrance, batt_meter_cust_data.cust_poweron_low_capacity_tolrance,
		 plugout_status);
sprintf(bootbuf_dod, "[dod_init][V0222]chr_exist=%d|boot_reason=%d|SW_OCV=%d SW_SOC=%d|SW_OCV_w=%d SW_SOC_w=%d|HW_OCV=%d HW_SOC=%d|RTC=%d|t=%d|%d %d|bat=%d\n",
                 charger_exist, get_boot_reason(), g_booting_vbat, SW_SOC, SW_OCV_with_charger, SW_SOC_save_with_charger, HW_OCV, HW_SOC, g_rtc_fg_soc, g_tbat, 
                 batt_meter_cust_data.cust_poweron_delta_capacity_tolrance, batt_meter_cust_data.cust_poweron_low_capacity_tolrance,
                 plugout_status);
//<2017/2/18-nickygao
...
void oam_init(void)
{
	...
	kal_bool charging_enable = KAL_FALSE;
       ...
       SW_OCV_with_charger = 5;	/* set avg times */
       ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_BAT_SENSE, &SW_OCV_with_charger);
	SW_SOC_with_charger = fgauge_read_capacity_by_v(SW_OCV_with_charger);
	SW_SOC_save_with_charger = SW_SOC_with_charger;
	...
	vbat_capacity = fgauge_read_capacity_by_v(g_booting_vbat);
       ...
       //<2016/12/1-nickygao, [LV3][MISC][COMMON][BSP][][]Add indicator for initial fuel gauge
       HW_OCV = gFG_voltage;
       HW_SOC = gFG_capacity_by_v;
       g_vbat_capacity = vbat_capacity;
	SW_SOC = g_vbat_capacity;
       charger_exist = bat_is_charger_exist();
       //<2016/12/1-nickygao, [LV3][MISC][COMMON][BSP][][]Decrease debug level for optimizing power-on speed
       bm_print(BM_LOG_FULL, "g_vbat_capacity=%d\n", g_vbat_capacity);
       //<2016/12/1-nickygao
       //<2016/12/1-nickygao
...
//<2017/2/18-nickygao, [LV3][MISC][COMMON][BSP][][]Update full battery voltage according to battery temperature
#define FULL_BATTERY_VOLTAGE_TOLERANCE	33
#define LV3_FULL_BATTERY_VOLTAGE	4360
//<2017/2/18-nickygao
void oam_run(void)
{
	int vol_bat = 0;
	...
	printk(bootbuf);
#endif
//<2016/10/24-nickygao, [LV3][MISC][COMMON][BSP][][]Add indicator for initial fuel gauge
        printk(bootbuf_dod);
//<2016/10/24-nickygao
	...
	fgauge_construct_table_by_temp();
       ...
       if (BMT_status.temperature < MIN_CHARGE_TEMPERATURE) {
	    batt_cust_data.full_battery_voltage = battery_profile_t0[0].voltage;
	    bm_print(BM_LOG_CRTI, "[%s]full_battery_voltage = %d tbat(%d) < low(%d)\n", 
			                                                    __func__, batt_cust_data.full_battery_voltage, BMT_status.temperature, MIN_CHARGE_TEMPERATURE);
	} else {
	    //batt_cust_data.full_battery_voltage = battery_profile_temperature->voltage;
	    //batt_cust_data.full_battery_voltage = battery_profile_temperature[0].voltage - FULL_BATTERY_VOLTAGE_TOLERANCE;
	    batt_cust_data.full_battery_voltage = LV3_FULL_BATTERY_VOLTAGE;
	    bm_print(BM_LOG_CRTI, "[%s]full_battery_voltage = %d tbat = %d\n", __func__, batt_cust_data.full_battery_voltage, BMT_status.temperature);
	}
       //<2016/12/1-nickygao
...
void table_init(void)
{
	...
	//<2017/2/17-nickygao, [LV3][MISC][COMMON][BSP][][]Print battery temperature
	g_tbat = temperature;
	//<2017/2/17-nickygao
...
#if defined(SOC_BY_SW_FG)
              //<2016/12/1-nickygao, [LV3][MISC][COMMON][BSP][][]Initialize HW FG to add charging and dischang current indicator
...
// + 20161110-Rody, [LV3][PORTING][COMMON][BATTERY][][]create temp_switch node
/* ///////////////////////////////////////////////////////////////////////////////////////// */
/* // Create File For Temperature : contrul temperature on(1)/off(0)  */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
static ssize_t show_temp_switch(struct device *dev, struct device_attribute *attr,char *buf)
{
	unsigned int write_data;
	if (temperature_flag ==KAL_FALSE) {
		//FALSE =0
		printk("[Temp_switch] temp_switch = KAL_FALLSE, BMT_status.temperswitch_enable= %d\n", BMT_status.temperswitch_enable);
		write_data =1;
	} else if( temperature_flag ==KAL_TRUE) {
		// TRUE =1
		printk("[Temp_switch] temp_switch = KAL_TRUE, BMT_status.temperswitch_enable= %d\n", BMT_status.temperswitch_enable);
		write_data =0;
	}
...
static ssize_t store_temp_switch(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int read_data;
 
	kstrtouint(buf, 10, &read_data);
	printk("[Temp_switch] temp_switch = %d\n", read_data);
	if (read_data == 0 ) {
		printk("Rody: temp_switch = TRUE\n");
		temperature_flag=KAL_TRUE; // 1
		BMT_status.temperswitch_enable=1;
	} else {
		printk("[Temp_switch]temp_switch = KAL_FALSE\n");
		temperature_flag=KAL_FALSE; // 0
		BMT_status.temperswitch_enable=0;
	}
 
	return size;
}
 
static DEVICE_ATTR(temp_switch, 0664, show_temp_switch, store_temp_switch);
// - 20161110-Rody, [LV3][PORTING][COMMON][BATTERY][][]create temp_switch node
/* ============================================================ // */
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_temp_switch);
	// - 20160921-Rody, [LV3][PORTING][COMMON][BATTERY][][]create temp_switch node
