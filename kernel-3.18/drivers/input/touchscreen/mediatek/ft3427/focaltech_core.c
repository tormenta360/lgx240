...
//extern int fts_test_result(int value);
...
unsigned char suspended_flag=0; // calvin ++
...
static int fts_read_Touchdata(struct ts_event *data)
{
	...
	//mutex_unlock(&i2c_access);	
    ret = data->touchs; // 20161216 calvin +
	memset(data, 0, sizeof(struct ts_event));
    data->touchs = ret; // 20161216 calvin +
	data->touch_point = 0;	
	data->touch_point_num=buf[FT_TOUCH_POINT_NUM] & 0x0F;
	//[Arima JimCheng 20170112] removed unused log  ++
...
static int fts_report_value(struct ts_event *data)
 {
    ...
    // 20161019 calvin ++ , fix DUT was hang in 'Android is starting...' interface and auto restart
    #if 0
...
/*int fts_test_result(int value)
{
  
   fts_wq_data->test_result=value;
...
//20160527 tracy add for 5700 auto test function start
int fts_i2c_test_write(unsigned char *wBuf, int wLen)
	{
...
int fts_i2c_test_read(unsigned char *wBuf, int wLen, unsigned char *rBuf, int rLen)
{
	if (NULL == fts_i2c_client) {
		return -1;
	}
...
//20160527 tracy add for 5700 auto test function end

/*[LG LV3][zihweishen] store touch firmware version to device node fwversion 2016/08/16 begin*/
static ssize_t show_Focal_FW(struct device_driver *ddri, char *buf)  
{  
	ssize_t   ret = 0;
	u8 ver;
	fts_read_reg(i2c_client, 0xa6, &ver);
	fts_read_reg(fts_i2c_client, 0xa6, &ver);
	sprintf(buf, "Focal_ver0x%02X\n", ver);
	ret = strlen( buf) + 1;
	return ret; 
}  

static DRIVER_ATTR(fwversion, 0664, show_Focal_FW, NULL);
/*[LG LV3][zihweishen] 2016/08/16 end*/
...
// All Fingers Up
	...
	//enable_irq(touch_irq); calvin --
#endif

    suspended_flag = 0; // calvin ++
}
...
//[LG D6][zihweishen] knock on funtion porting 2016/09/1 begin
static void tpd_suspend(struct device *h)
{
	int retval = TPD_OK;
    ...
    if(suspended_flag ==1)
    {
    	printk("TPD suspended_flag == 1, return!! \n");
        return;
    }
    else
    {
		printk("TPD suspended_flag == 0\n");
    }
    suspended_flag =1;
	// calvin --
	...
	#endif
//[LG D6][zihweishen] 2016/09/1 end
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  //+ murphy 2017.01.24  change I2C speed to 400KHz
  if(i2c_client->timing != 400)
  {
    i2c_client->timing = 400;
    printk("frank_zhonghua:i2c_client_FT->timing=%d\n",i2c_client->timing);
  }
  //- murphy 2017.01.24  change I2C speed to 400KHz
...
/*[LG LV3][zihweishen] store touch firmware version to device node fwversion 2016/08/16 begin*/
		err1=driver_create_file(&(tpd_device_driver.platform_driv_addr->driver), &driver_attr_fwversion);
/*[LG LV3][zihweishen] 2016/08/16 end*/
