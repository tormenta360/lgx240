...
//<2016/11/04-Tonywu, [D6][PORTING][COMMON][SENSOR][][]G sensor Driver update
#define DRIVER_VERSION				"V60.97.05.01"
//>2016/11/04-Tonywu
//<2017/02/17-Tonywu, Sensor.close G sensor debug information
#define GSE_DEBUG_ON          		0
//>2017/02/17-Tonywu
//<2016/09/05-Tonywu, Sensor.close G sensor debug information
...
#define MXC400X_CUSTOMIZATION_CALI
//<2016/10/03-Tonywu, [D6][BUG][COMMON][SENSOR][][] Fix resume/suspend Time of G sensor
#define  USE_DELAY
//>2016/11/03-Tonywu
...
#endif
#ifdef MXC400X_CUSTOMIZATION_CALI
static mm_segment_t oldfs;
//<2016/12/20-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Modify calibration file path & read file
#define CALI_FILE_PATH "/protect_f/Gsensor_calibration.dat"
//>2016/12/20-Tonywu
#endif
...
//<2016/12/20-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Modify calibration file path & read file
#ifdef MXC400X_CUSTOMIZATION_CALI
static int mxc400x_read_cali_data(struct i2c_client *client);
//<2016/12/22-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Fix G sensor driver file open process
static int first_read_cali = 0;
//<2016/12/22-Tonywu
#endif
//>2016/12/20-Tonywu
...
#ifdef USE_DELAY
	if(delay_state){
//<2016/11/16-Tonywu, [D6][BUG][COMMON][SENSOR][][] Fix CTS fail
		msleep(150);
//>2016/11/16-Tonywu
		delay_state = 0;
	}
...
#ifndef MXC400X_CUSTOMIZATION_CALI
...
#endif
#ifdef MXC400X_CUSTOMIZATION_CALI
	cali[MXC400X_AXIS_X] = 0;
	cali[MXC400X_AXIS_Y] = 0;
	cali[MXC400X_AXIS_Z] = 0;
#else
...
#ifdef MXC400X_CUSTOMIZATION_CALI
static ssize_t store_cust_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	struct i2c_client *client = mxc400x_i2c_client;
	struct mxc400x_i2c_data *data = i2c_get_clientdata(client);
	int err,x,y,z;
	...
	{
		dat[MXC400X_AXIS_X] = x * data->reso->sensitivity / GRAVITY_EARTH_1000;
		dat[MXC400X_AXIS_Y] = y * data->reso->sensitivity / GRAVITY_EARTH_1000;
		dat[MXC400X_AXIS_Z] = z * data->reso->sensitivity / GRAVITY_EARTH_1000;
		if((err = MXC400X_WriteCalibration(client, dat)))
		{
			GSE_ERR("write calibration err = %d\n", err);
		}
...
#ifdef MXC400X_CUSTOMIZATION_CALI
static DRIVER_ATTR(node_CALI,	S_IWUSR | S_IRUGO, NULL,	store_cust_cali_value);
#endif
...
static int mxc400x_enable_nodata(int en)
{
	...
	#ifdef MXC400X_CUSTOMIZATION_CALI
	//<2016/12/22-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Fix G sensor driver file open process
	if( first_read_cali == 0)
	{
		mxc400x_read_cali_data(obj_i2c_data->client);
		first_read_cali = 1;
		GSE_INFO("First read Calibration file is done!!!");
	}
	#endif
	//<2016/12/20-Tonywu
...
#ifdef MXC400X_CUSTOMIZATION_CALI
static struct file *open_file(const char *path, int flag, int mode)
{
	struct file *fp = NULL;
...
{
	struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);
	static struct file *fp_file;
	int cali_data[3] = {0};
	int err = 0,i = 0;
	char buf[64] = {0};
//Set file opreate function environment

	fp_file = open_file(CALI_FILE_PATH, O_RDONLY, 0);
	...
	if (fp_file == NULL)
	{
		...
		cali_data[1] = 0;
		cali_data[2] = 0;
		GSE_ERR("fail to open calibration file:%s,Offset set to 0\n", CALI_FILE_PATH);
		goto exit_write_cali_data;
	}
	...
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	err = read_file(fp_file, buf, sizeof(buf));
	if (err > 0)
		GSE_INFO("Read Cali data successfullybuf:%s\n", buf);
	else
	{
		GSE_INFO("read file error %d\n", err);
		return -1;
	}
	...
	err = sscanf(buf, "%d %d %d", &cali_data[MXC400X_AXIS_X], &cali_data[MXC400X_AXIS_Y], &cali_data[MXC400X_AXIS_Z]);
	GSE_DEBUG("cust cali data(G): %d %d %d\n", cali_data[MXC400X_AXIS_X], cali_data[MXC400X_AXIS_Y], cali_data[MXC400X_AXIS_Z]);

	cali_data[MXC400X_AXIS_X] = cali_data[MXC400X_AXIS_X] * obj->reso->sensitivity / GRAVITY_EARTH_1000;
	cali_data[MXC400X_AXIS_Y] = cali_data[MXC400X_AXIS_Y] * obj->reso->sensitivity / GRAVITY_EARTH_1000;
	cali_data[MXC400X_AXIS_Z] = cali_data[MXC400X_AXIS_Z] * obj->reso->sensitivity / GRAVITY_EARTH_1000;

	GSE_DEBUG("cust cali data(Count): %d %d %d\n", cali_data[MXC400X_AXIS_X],
		cali_data[MXC400X_AXIS_Y], cali_data[MXC400X_AXIS_Z]);
exit_write_cali_data:
	MXC400X_WriteCalibration(client, cali_data);
...
//<2016/12/20-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Modify calibration file path & read file
//#ifdef MXC400X_CUSTOMIZATION_CALI
	//mxc400x_read_cali_data(new_client);
//#endif
//<2016/12/20-Tonywu
	GSE_INFO("%s: OK\n", __func__);
	return 0;
