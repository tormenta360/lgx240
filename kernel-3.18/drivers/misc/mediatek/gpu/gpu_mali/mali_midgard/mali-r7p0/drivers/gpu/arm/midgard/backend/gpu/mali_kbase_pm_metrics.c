...
//<2017/02/06-samhuang, add for count GPU loading
#define AVG_COUNT 100
unsigned int gGPUCount = 0;
unsigned int gGPUTotal = 0;
unsigned int gGPUAVG = 0;
//>2017/02/06-samhuang
...
void MTKCalGpuUtilization(unsigned int* pui32Loading , unsigned int* pui32Block,unsigned int* pui32Idle)
{
	...
	//<2017/02/06-samhuang, add for count GPU loading
	#if 1
	if (gGPUCount <= AVG_COUNT) {
		gGPUTotal += utilisation;
		if( gGPUCount == AVG_COUNT ) {
			gGPUAVG = gGPUTotal / AVG_COUNT;
			gGPUCount = 0;
			gGPUTotal = 0;
		} else {
			gGPUCount++;
		}
		//printk("\n[Arima] GPU Loading = %d/%d\n", gGPUCount,gGPUAVG);
	}
	#endif
	//>2017/02/06-samhuang
