...
static unsigned int itson_rx(const struct nf_hook_ops *ops,
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,13,0)
static unsigned int itson_tx(const struct nf_hook_ops *ops,
#else
...
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,14,0) || defined CONFIG_UIDGID_STRICT_TYPE_CHECKS
	minfo.uid = NETLINK_CB(skb_2).creds.uid.val;
	minfo.gid = NETLINK_CB(skb_2).creds.gid.val;
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,14,0) || defined CONFIG_UIDGID_STRICT_TYPE_CHECKS
			sock_uid = SOCK_INODE(skb->sk->sk_socket)->i_uid.val;
#else
...
ItsOnDoWork(int index, int delay)
{
	...
	if (index >= 0) {
		cancel_delayed_work(&workers[index]);
