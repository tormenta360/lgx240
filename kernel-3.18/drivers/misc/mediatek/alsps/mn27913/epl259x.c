...
#define ARIMA_DYNAMIC_CAL 0///<2015/05/03-ShermanWei, Enable Dynamic Cal
#define PS_BOOT_K 0///1
#define ALS_DYN_INTT 0///1
/******************************************************************************
 *  PS_BOOT_K
 ******************************************************************************/
#if PS_BOOT_K
 ...
 *  ALS / PS define
 ******************************************************************************/
//<2016/11/17-Tonywu,[D6][PORTING][COMMON][SENSOR][][] change L sensor calibration default value
#define LUX_PER_COUNT	130  //D6 PP2:200//ALS lux per count SM20:35
//>2016/11/17-Tonywu
/*ALS interrupt table*/
...
int dynamic_intt_init_idx = 1;///0;	//initial dynamic_intt_idx
int c_gain;
int dynamic_intt_lux = 0;
uint16_t dynamic_intt_high_thr;
uint16_t dynamic_intt_low_thr;
uint32_t dynamic_intt_max_lux = 20000;////18000;
uint32_t dynamic_intt_min_lux = 0;
uint32_t dynamic_intt_min_unit = 1000;
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_1024, EPL_ALS_INTT_256/* EPL_ALS_INTT_1024 */};
static int als_dynamic_intt_value[] = {1024, 256/* 1024 */}; //match als_dynamic_intt_intt table
static int als_dynamic_intt_gain[] = {EPL_GAIN_MID, EPL_GAIN_LOW};
static int als_dynamic_intt_high_thr[] = {60000, 60000};
static int als_dynamic_intt_low_thr[] = {200, 1000/* 6000 */};
...
static int arima_als_calibration_read(struct epl_sensor_priv *epl_data)
{
	...
	if(cal_value > 0)
	{
	    ...
	    ///2016/06/15 shermanwei shift data for support 20000Lux
	    epl_sensor.als.factory.lux_per_count = (epl_data->lux_per_count);
	    #if ALS_DYN_INTT
	    c_gain = (epl_data->lux_per_count);
	    #endif
    }
	else
	{
...
//<2016/10/18-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Fix sun light problem
//<2016/10/11-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Fix sun light problem
    epl_sensor.als.als_std = EPL_ALS_STD;//EPL_STD;//EPL_PRE;
//>2016/10/11-Tonywu
//>2016/10/18-Tonywu
/* Porting ICE */
...
//<2016/10/18-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Fix sun light problem
    epl_sensor.ps.ps_std = EPL_PS_STD;//EPL_STD;///EPL_PRE; EMI Flank suggestion ///20160809
//>2016/10/18-Tonywu
/* Porting ICE */
...
////..    struct i2c_client *client = epld->client;
    ...
    u8 buf[4];
	//<2016/12/22-Tonywu, [D6][PORTING][COMMON][SENSOR][][] follow LG gray card
	low_thd/=3;
	high_thd/=3;
	//>2016/12/22,
...
static ssize_t epl_sensor_show_reg(struct device_driver *ddri, char *buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len, "chip id REG 0x20 value = 0x%x\n", i2c_smbus_read_byte_data(client, 0x20));
    len += snprintf(buf+len, PAGE_SIZE-len, "chip id REG 0x21 value = 0x%x\n", i2c_smbus_read_byte_data(client, 0x21));
...
static ssize_t epl_sensor_show_pdata(struct device_driver *ddri, char *buf)
{
      ...
      //<2016/10/04-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Sensor porting
	  struct epl_sensor_priv *epld = epl_sensor_obj;
	  
	  if ( (test_bit(CMC_BIT_PS, &epld->enable)) && (atomic_read(&epld->ps_suspend)==0) )
	  {	
		  ps_raw = epl_sensor.ps.data.data;
		  APS_LOG("[%s]: PS already on,ps_raw=%d \r\n", __func__, ps_raw);
	  }
	  else
	  {	
...
//sherman modify_2
	...
	clear_bit(CMC_BIT_PS, &epld->enable);
	epl_sensor_update_mode(epld->client);
//<2016/10/27-Tonywu, [D6][PORTING][COMMON][SENSOR][][] P/L Sensor Calibration
	memset(high, NULL, sizeof(high));
	high_length = 4;
...
//	ch1 = (u16)ch1_all/count;
    ...
    epl_sensor_update_mode(epld->client);
//<2016/10/27-Tonywu, [D6][PORTING][COMMON][SENSOR][][] P/L Sensor Calibration
	memset(ctalk, NULL, sizeof(ctalk));
	ctalk_length = 4;
...
static int als_get_data(int* value, int* status)
{
    ...
    epl_sensor_read_als(obj->client);
	//<2016/10/18-Tonywu, [D6][PORTING][COMMON][SENSOR][][] Fix sun light problem
	if(epl_sensor.als.saturation == 0x20)
	{
		epl_sensor.als.data.channels[1] = 65535;
	}
	//>2016/10/18-Tonywu
...
///    if((i2c_smbus_read_byte_data(client, 0x21)) != EPL_REVNO){ //check chip
	...
	printk("sherman +++++ epl_sensor_i2c_probe.1 renvo=0x%x +++++ \n", renvo);
