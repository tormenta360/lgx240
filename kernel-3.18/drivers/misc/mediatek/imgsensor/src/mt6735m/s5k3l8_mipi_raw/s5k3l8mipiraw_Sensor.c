...
/****************************   Modify end    *******************************************/
#ifdef DUMP_OTP
extern int  dump_lsc_data(int page ,int start, int end);
#endif

    //Neil 20161212   read otp to globle var S5K3L8_OTPData[]+
    
#define YSL_PRE_LOAD_OTP
    
#ifdef YSL_PRE_LOAD_OTP
    #define S5K3L8_OTP_START_ADDR 0x0a04
    #define S5K3L8_OTP_SIZE 50
    extern  int QT_S5K3L8_Raed_OTP_Table(u32 offset, BYTE *data,  u32 size );	
    u8 S5K3L8_OTPData[S5K3L8_OTP_SIZE]={0};
#endif	
    //Neil 20161212   read otp to globle var S5K3L8_OTPData[]-
...
#ifdef DUMP_OTP
static void read_otp_info(void)
{
	 LOG_INF("read_otp_info S =====================================\n");
	...
	// write_cmos_sensor(0x0b00, 0x0100);//shading on/off(0x0000:disable, 0x0100 enable)
	 ...
	 write_cmos_sensor(0x6028, 0x2000); //sarah add 
	 write_cmos_sensor(0x602A, 0x0CF4); //sarah add 
	 write_cmos_sensor(0x6F12, 0x0100); //sarah add 
	 write_cmos_sensor(0x6028, 0x4000); //sarah add 
	 write_cmos_sensor(0x0B00, 0x0100); //sarah add 
	 //write_cmos_sensor(0x0100, 0x0000); //sarah mark off 
	 write_cmos_sensor(0x0100, 0x0100); //sarah add 
	...
	// dump_otp_info();
	// start_read_otp(page);
	 dump_lsc_data(0x0100, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1200, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1300, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1400, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1500, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1600, 0x0A04, 0x0A11);
	 dump_lsc_data(0x1700, 0x0A12, 0x0A43);
	 dump_lsc_data(0x1800, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1900, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1A00, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1B00, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1C00, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1D00, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1E00, 0x0A04, 0x0A43);
	 dump_lsc_data(0x1F00, 0x0A04, 0x0A43);
	 

	
	 
	 LOG_INF("read_otp_info E =====================================\n");
...
{
  LOG_INF("E\n");
    write_cmos_sensor(0x0100, 0x0000); //sarah add
...
//$MV1[MCLK:24,Width:2104,Height:1560,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
  ...
  write_cmos_sensor(0x0B08, 0x0100); //BPC enable by norman@20161220

  write_cmos_sensor(0x6028, 0x2000); //sarah add 
  write_cmos_sensor(0x602A, 0x0CF4); //sarah add
  write_cmos_sensor_byte(0x6F12, 0x01); //sarah add
  write_cmos_sensor(0x6028, 0x4000); //sarah add 
  write_cmos_sensor(0x0B00, 0x0107); //sarah add
...
//  write_cmos_sensor(0x0B00, 0x0007); //0x0007 sarah test
//	write_cmos_sensor(0x0B00, 0x0100); //sarah add
  write_cmos_sensor(0x316A, 0x00A0);
  write_cmos_sensor(0x0100, 0x0100);
...
//write_cmos_sensor(0x0B00, 0x0007); //0x0007 sarah test	//LSC_Off
//	write_cmos_sensor(0x0B00, 0x0100); //sarah add
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		//read_imx135_otp_mtk_fmt();
  		...
  		QT_S5K3L8_Raed_OTP_Table(S5K3L8_OTP_START_ADDR, S5K3L8_OTPData,S5K3L8_OTP_SIZE);
      		int j;
      		for (j=0;j<S5K3L8_OTP_SIZE;j++){
	  		LOG_INF("S5K3L8_OTPData[%d]=0x%x.\n",j,S5K3L8_OTPData[j] );
